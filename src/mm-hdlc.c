...
/* HDLC frames end with a 3-byte trailer consisting of the 16-bit CRC of the
 * data plus an ending async control character (0x7E).  The data and the CRC are
 * escaped before adding the trailing control character (0x7E) to ensure the
 * control character and escape marker (0x7D) never appear in the data itself.
 */
...
static guint16
crc_update (guint16 crc, guint8 val)
{
...
framed_len (GByteArray *bytes)
{
    ...
    for (i = 0; i < bytes->len; i++) {
        if (bytes->data[i] == HDLC_CONTROL || bytes->data[i] == HDLC_ESC_CHAR)
            required++;
    }
    return required;
}
 ...
 * escape markers and a 16-bit CRC.
 *
...
mm_hdlc_encapsulate (GByteArray *bytes, GError **error)
{
    ...
    framed = g_byte_array_sized_new (len);
 ...
 * from @bytes.
 * @out_need_more: on return
 * @error: on return contains an error or %NULL if no error occurred
 *
 * Unframes @bytes from an HDLC frame and validates the CRC. Note that both
 * the returned array and @error can be %NULL if @out_need_more is %TRUE or
 * @out_bytes_used is greater than zero. If no valid HDLC frame can be found,
 * @out_need_more will be %TRUE on return indicating more data is required.
 * If any HDLC framing is found, @out_bytes_used will be greater than zero
 * indicating that this amount of data can be discarded from @bytes.  If
 * any HDLC framing is found but the framing cannot be validated, or the
 * calculated CRC does not match the frame CRC, an error will be returned
 * in @error.
 *
 * Returns: the decapsulated data from an HDLC frame if one was found and valid,
 * otherwise %NULL
 */
GByteArray *
                     ...
                     GError **error)
{
    ...
    /* Find the trailing HDLC control character */
    for (i = 0; i < bytes->len; i++) {
        if (bytes->data[i] == HDLC_CONTROL) {
            /* If the control character shows up in a position before a valid
             * HDCL frame (4 bytes; one of data, two CRC, one control character)
    ...
    if (frame_len == 0) {
        /* HDLC control char not yet found */
        if (out_need_more)
            *out_need_more = TRUE;
        return NULL;
    }
